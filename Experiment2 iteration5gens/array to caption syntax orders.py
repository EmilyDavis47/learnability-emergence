...
df = pd.read_csv('/Users/emilydavis/Desktop/paper data/Experiment2 iteration5gens/writeInsIterNew.csv')
def makeGrammar(df): 
       ...
       for b2 in range(len(flipV)): #flipping meaning of vertical adposition
           for b3 in range(len(prepSwap)): #swapping assignment of preps to spatial meanings
              ...
              for i in range(len(users)): #each user's noun and preposition sets
                  consonants = list(df[df.workerId == users[i]]['consonants'])[0]
                  consonantsPrep= 'mr'
                  newColGrammarUser = []
  ...
  return df
df2 = makeGrammar(df)
df2.to_csv('/Users/emilydavis/Desktop/paper data/Experiment2 iteration5gens/writeInsIterNewGrammar.csv')       
